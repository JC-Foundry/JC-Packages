...
|---------|----------|------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7.1.5   | JC.CAP   | 8 September 2026 | Sign-out no longer sends `id_token_hint`. CAP ends the session on `client_id` and `post_logout_redirect_uri` alone, and the hint was a whole identity token carrying the user's roles, which OpenIddict then copied into the state token as well — around four kilobytes of query string, past IIS's 2048-character `maxQueryString`. The sign-out URL is now under a kilobyte. A handler also strips any hint an application sets itself out of the state token, so it can never be sent twice. |
